...
pub async fn get_value<C>(conn: &mut C, key: &str) -> Result<u64, Error>
where
    C: ConnectionLike,
{
...
pub async fn set_value<C>(conn: &mut C, key: &str, value: &u64) -> Result<(), Error>
where
    C: ConnectionLike,
{
...
mod tests {
    ...
    async fn it_return_succeed_when_get_is_called() {
        ...
        let result = run_query(get_value(&mut conn, &key)).await;
    ...
    async fn it_return_succeed_when_set_is_called() {
        ...
        let result = run_query(set_value(&mut conn, &key, &value)).await;
